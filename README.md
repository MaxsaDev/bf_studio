...
3. On the payment screen the buyer chooses pickup at the studio, an electronic BFCard ("Електронний", sent by the staff after payment) or Nova Poshta delivery to a recipient (name, phone, city, branch or parcel locker; `/api/np/*` proxy the service's lookups).
4. Browser POSTs `{ items: [{ certificateId, variantId, addons, qty }], name, phone, delivery }` to `/api/create-invoice`.
5. The route validates every item, **resolves the total from the catalog** (client-supplied amounts are never accepted), rate-limits per IP and sends the priced order to bf-back-v2 `POST /api/v1/certificates/orders`, which stores it and creates the WayForPay invoice on the studio merchant. The invoice is itemised (`lib/receipt.ts`): every card is its own line on the buyer's fiscal receipt, followed by that card's add-ons ("Додаток до масажу «Масаж стоп»"), so the buyer sees which card each add-on belongs to. The delivery choice never appears there - it goes only to the staff. The response carries the order's `clientToken`.
