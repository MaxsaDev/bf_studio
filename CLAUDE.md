...
- **`app/`** - Next.js app router pages
  ...
  - `api/create-invoice/route.ts` - Server-side order proxy. Validates `{ items: [{ certificateId, variantId, addons, qty }], name, phone, delivery }` with zod, **re-resolves every price from `data/certificates.ts` + `data/addons.ts`** (a client-supplied amount is never accepted), rate-limits per IP (`lib/rate-limit.ts`), appends the delivery choice to the description and sends ONE order for the whole cart (priced item snapshot + buyer + delivery + itemised receipt lines `products` from `lib/receipt.ts`) to bf-back-v2 `POST /api/v1/certificates/orders`, which stores it and creates the WayForPay invoice. Answers `{ url, orderReference, clientToken }`
...
### Data Layer
- **`data/addons.ts`** - The six add-ons ("додатки": foot/hand/head massage, aroma oils, candles, extra time) with title, fiscal name, price and sticker icon (`public/addons/*.svg`), plus the sheet presets `ADDONS_SINGLE` (1 of each) and `ADDONS_COURSE` (0-6 of each). Source of truth is the owner's "bf-card-additions" sheet. `fiscalTitle` ("Додаток до масажу «Масаж стоп»") is printed on the buyer's fiscal receipt; keep «» quotes, never straight `"` (they would need escaping in WayForPay's stitched callback JSON), and no `[ ] & = + %`
...
- `receipt.ts` - `buildReceiptLines(cart)` → the WayForPay invoice lines, which WayForPay prints on the buyer's fiscal receipt: each card under its catalog description at its discounted price, followed right by that card's own add-ons under their `fiscalTitle` (count = add-on qty × cards of the line). Never merged across cards: the buyer must see which card each add-on belongs to. Never any delivery text. `receiptTotalKop()` checks they add up to the total; if they ever don't, the route falls back to a single-line invoice instead of failing the sale
...
- `lib/__tests__/receipt.test.ts` covers the fiscal receipt lines (add-ons grouped under their card, totals, no delivery text, safe characters)
- `lib/__tests__/create-invoice-route.test.ts` covers the payment proxy: validation of `items[]`, server-side price resolution (client `amount` ignored), multi-line totals, per-card add-ons, the receipt lines sent as `products`, upstream error mapping, rate limiting. `fetch` is stubbed and `.env.local` is not loaded, so no real invoice is ever created by tests
...
- The whole cart is ONE WayForPay invoice with itemised lines (`products`, see `lib/receipt.ts`): one per card, then one per add-on of that card under its fiscal name. WayForPay prints them on the buyer's fiscal receipt, so they must never carry delivery or other service text. `orderDescription` (cards, add-ons and delivery in words, capped at 250 chars, see `describeCart` / `withDeliveryDescription`) is only for the staff: order row, admin SMS, Telegram
