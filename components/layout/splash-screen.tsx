...
export function SplashScreen() {
  ...
  useEffect(() => {
    // Returning visitors in the same session skip the splash
    const skip = hasSeenSplash();

    const timer = setTimeout(
      () => {
        if (!skip) markSplashSeen();
        setWasSkipped(skip);
        setIsLoading(false);
      },
      skip ? 0 : SPLASH_DURATION_MS
    );
    ...
    <AnimatePresence>
      {isLoading && (
        <motion.div
          ...
          initial={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          transition={{ duration: wasSkipped ? 0.2 : 0.8, ease: "easeInOut" }}
        ...
        >
          <div className="relative flex flex-col items-center">
            ...
            >
              <div className="text-6xl sm:text-8xl font-serif font-bold tracking-tighter text-stone-900 dark:text-stone-100">
                BODY FACTORY
              </div>
              <motion.div
                initial={{ width: 0 }}
